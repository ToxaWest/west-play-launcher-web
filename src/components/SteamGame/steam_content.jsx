...
const GameContent = () => {
    ...
    const ach = null;
    const [game, fetchGame, loading] = useActionState(async () => {
        return await electronConnector.getGameByID({source: 'steam', id})
    }, {})

    useEffect(() => {
        startTransition(fetchGame)
        electronConnector.getPlayTime({source: 'steam', id}).then(d => {
            if (d) {
